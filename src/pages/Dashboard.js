      ...
      {showWelcomeModal && (
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-50">
          <div className="bg-white w-full sm:max-w-2xl md:max-w-4xl sm:mx-4 sm:rounded-3xl rounded-t-3xl max-h-[90vh] sm:max-h-[85vh] overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="bg-gradient-to-r from-yellow-400 to-amber-500 px-4 py-4 sm:p-6 text-white text-center flex-shrink-0">
              <div className="flex items-center justify-center mb-2 sm:mb-4">
                <div className="bg-white rounded-full w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center shadow-lg">
                  <span className="text-2xl sm:text-4xl font-bold text-yellow-500">P</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 mt-3 sm:mt-4">
                {[1, 2, 3, 4].map((step) => (
                  <div
                    key={step}
                    className={`h-1.5 sm:h-2 rounded-full transition-all ${step === currentStep ? "w-6 sm:w-8 bg-white" : "w-1.5 sm:w-2 bg-yellow-200"}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 min-h-0">
              {onboardingError && (
                <div className="mb-3 text-xs sm:text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg p-2 sm:p-3">
                  {onboardingError}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
                {currentStep === 1 && cuisineOptions.map((option) => (
                  <div
                    ...
                    className={`relative rounded-xl overflow-hidden cursor-pointer transition-all active:scale-95 ${
                      selectedCuisines.includes(option.id) ? "ring-2 sm:ring-4 ring-yellow-400" : ""
                    }`}
                  >
                    ...
                    {selectedCuisines.includes(option.id) && (
                      <div className="absolute top-1 right-1 sm:top-2 sm:right-2 bg-yellow-400 rounded-full p-0.5 sm:p-1">
                ...
                {currentStep === 2 && dislikeOptions.map((option) => (
                  <div
                    ...
                    className={`relative rounded-xl overflow-hidden cursor-pointer transition-all active:scale-95 ${
                      selectedDislikes.includes(option.id) ? "ring-2 sm:ring-4 ring-red-400" : ""
                    }`}
                  >
                    <img src={option.image} alt={option.name} className="w-full h-20 sm:h-28 object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-2">
                    ...
                    {selectedDislikes.includes(option.id) && (
                      <div className="absolute top-1 right-1 sm:top-2 sm:right-2 bg-red-500 rounded-full p-0.5 sm:p-1">
                ...
                {currentStep === 3 && dietOptions.map((option) => (
                  <div
                    ...
                    className={`relative rounded-xl overflow-hidden cursor-pointer transition-all active:scale-95 ${
                      selectedDiets.includes(option.id) ? "ring-2 sm:ring-4 ring-yellow-400" : ""
                    }`}
                  >
                    <img src={option.image} alt={option.name} className="w-full h-20 sm:h-28 object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-2">
                ...
                {currentStep === 4 && allergenOptions.map((option) => (
                  <div
                    ...
                    className={`relative rounded-xl overflow-hidden cursor-pointer transition-all active:scale-95 ${
                      selectedAllergens.includes(option.id) ? "ring-2 sm:ring-4 ring-red-400" : ""
                    }`}
                  >
                    <img src={option.image} alt={option.name} className="w-full h-20 sm:h-28 object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-2">
            ...
            <div className="bg-gray-50 px-4 py-3 sm:p-4 flex items-center justify-between border-t flex-shrink-0" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
              <button
                onClick={handleSkip}
                className="px-3 sm:px-6 py-2 text-gray-500 hover:text-gray-700 font-medium text-sm sm:text-base transition"
              ...
              <button
                onClick={handleNext}
                className="px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-yellow-400 to-amber-500 text-white font-bold text-sm sm:text-base rounded-xl shadow-lg transition active:scale-95 disabled:opacity-70"
                disabled={savingOnboarding}
              >
