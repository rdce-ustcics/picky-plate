...
const brand = { primary: "#FFC42D", text: "#2b2b2b", bg: "#F6F6F7" };
...
export default function ChatBot() {
  ...
  // Loader: always try server. Only use LocalStorage fallback if allowed.
  useEffect(() => {
    (async function loadChats() {
      ...
      try {
        ...
        if (Array.isArray(list) && list.length) {
          ...
          setChats(uiChats);
          setActiveChatId(null);
          // keep anon cache in sync when logged out
          if (!isAuthenticated) saveChatsToLocal(uiChats);
          return;
        }

        // No server chats; if logged out and allowed, try LocalStorage
        if (canUseLocal) {
        ...
        // On error, only fallback to LocalStorage when logged out and not suppressed
        if (canUseLocal) {
          const cached = loadChatsFromLocal();
          if (Array.isArray(cached) && cached.length) {
            setChats(cached.map((c) => ({ ...c, messages: c.messages || [] })));
  ...
  // Keep anon cache in sync (but never during suppression)
  useEffect(() => {
        ...
        <div className={`chat-sidebar ${showSidebar ? "show" : ""}`} style={{ width: 256, minWidth: 256, background: "#fff", borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", flexShrink: 0, height: "100vh", position: "sticky", top: 0 }}>
          ...
          <div style={{ marginTop: 24, padding: "0 12px", flex: 1, overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {chats.map((chat) => (
                <div key={chat.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", padding: "6px 8px", borderRadius: 8, background: activeChatId === chat.id ? "#f3f4f6" : "transparent" }}>
                  <button
                    onClick={() => selectChat(chat.id)}
                    style={{ flex: 1, textAlign: "left", border: "none", background: "transparent", color: activeChatId === chat.id ? "#111827" : "#6b7280", fontWeight: activeChatId === chat.id ? 500 : 400, fontSize: 14, cursor: "pointer", padding: "8px 6px" }}
                  >
                    {chat.title}
                  </button>
              ...
              <div style={{ width: "100%", maxWidth: 672, marginTop: 32 }}>
                <div style={{ position: "relative" }}>
                  <input
                    ...
                    style={{ width: "100%", borderRadius: 16, border: "1px solid #d1d5db", background: "white", padding: "14px 56px 14px 20px", fontSize: 14, outline: "none", boxShadow: "0 1px 2px rgba(0, 0, 0, 0.05)", boxSizing: "border-box" }}
                    placeholder="Type or Ask anything"
                  />
                  <button
                    onClick={() => { if (input.trim()) createNewChat(input); }}
                    style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", borderRadius: 12, padding: 10, background: brand.primary, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                  >
                ...
                {activeChat?.messages?.length ? (
                  activeChat.messages.map((m, idx) => (
                    <div key={idx} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", alignItems: "flex-start", gap: 8 }}>
                      ...
                      <div style={{ maxWidth: "85%", borderRadius: 14, padding: "10px 14px", fontSize: 14, lineHeight: 1.5, background: m.role === "user" ? "#ffffff" : "#FFF7DA", border: m.role === "user" ? "1px solid #e5e7eb" : "1px solid #FEF3C7", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
                        {m.content}
                      </div>
                    </div>
                  ))
              ...
              <div style={{ padding: "12px 16px", borderTop: "1px solid #e5e7eb" }}>
                <div style={{ position: "relative", maxWidth: 768, margin: "0 auto" }}>
                  <input
                    ...
                    style={{ width: "100%", borderRadius: 16, border: "1px solid #e5e7eb", background: "white", padding: "10px 48px 10px 16px", fontSize: 14, outline: "none", boxSizing: "border-box" }}
                    placeholder="Type your message…"
                  />
                  <button
                    onClick={() => sendMessage()}
                    disabled={isTyping}
                    style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", borderRadius: 12, padding: 8, background: brand.primary, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", opacity: isTyping ? 0.8 : 1 }}
                    aria-label="Send"
